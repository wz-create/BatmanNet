...
def split(scaffolds_dict,labels,  weights, sample_size, random_seed=0):
...
def scaffold_split_fp(dataset,df, random_seed=8):
    """
    ...
    test_scaffold, test_index = split(all_scaffolds_dict, labels, weights, samples_size,
                                      random_seed=random_seed)
    training_scaffolds_dict = {x: all_scaffolds_dict[x] for x in all_scaffolds_dict.keys() if x not in test_scaffold}
    valid_scaffold, valid_index = split(training_scaffolds_dict, labels,weights, samples_size,
                                        random_seed=random_seed)
...
def cv_random_split(dataset, fold_idx = 0, fold_num=5, seed=0 ):

    skf = StratifiedKFold(n_splits=fold_num, shuffle = True, random_state = seed)

    labels = [data.target for data in dataset]
