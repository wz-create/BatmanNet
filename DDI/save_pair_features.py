...
from BatmanNet.util.utils import makedirs, load_features, save_features, save_pair_features
from BatmanNet.data.molfeaturegenerator import get_available_features_generators, get_features_generator
...
def generate_and_save_features(args: Namespace):
    ...
    data = get_data(path=args.data_path, dataset_name=args.dataset, max_data_size=None)
    ...
    if not os.path.exists(temp_save_dir):
        ...
        features1, temp_num1 = [], 0
        features2, temp_num2 = [], 0


    # Build features map function
    data = data[len(features1):]  # restrict to data for which features have not been computed yet
    mols1 = (d.smiles1 for d in data)
    mols2 = (d.smiles2 for d in data)


    if args.sequential:
        features_map1 = map(features_generator, mols1)
        features_map2 = map(features_generator, mols2)
    else:
        features_map1 = Pool(30).imap(features_generator, mols1)
        features_map2 = Pool(30).imap(features_generator, mols2)

    # Get features
    temp_features1 = []
    temp_features2 = []
    dataname = args.data_path.split('csv')[0] + 'npz'
    for i, feats in tqdm(enumerate(features_map1), total=len(data)):
        temp_features1.append(feats)

        # Save temporary features every save_frequency
        if (i > 0 and (i + 1) % args.save_frequency == 0) or i == len(data) - 1:
            save_features(os.path.join(temp_save_dir, f'{temp_num1}.npz'), temp_features1)
            features1.extend(temp_features1)
            temp_features1 = []
            temp_num1 += 1

    for i, feats in tqdm(enumerate(features_map2), total=len(data)):
        temp_features2.append(feats)

        # Save temporary features every save_frequency
        if (i > 0 and (i + 1) % args.save_frequency == 0) or i == len(data) - 1:
            save_features(os.path.join(temp_save_dir, f'{temp_num2}.npz'), temp_features2)
            features2.extend(temp_features2)
            temp_features2 = []
            temp_num2 += 1

    try:
        # Save all features
        save_pair_features(args.save_path, features1, features2)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', type=str, default="../data/twosides/raw/Drug_META_DDIE.db",
                        help='Path to data CSV')
    parser.add_argument('--features_generator', type=str, default='rdkit_2d_normalized',
                        choices=get_available_features_generators(),
                        help='Type of features to generate')
    parser.add_argument('--save_path', type=str, default="../data/biosnap/raw/biosnap.npz",
    ...
    # 这样的话，当不输入 --restart的时候，默认为False；输入 --restart的时候，才会触发True值。
