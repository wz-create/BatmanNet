...
from BatmanNet.data.moldataset import MoleculeDataset
...
def main():
    ...
    parser.add_argument('--save_dir', type=str, default="./model/twosides",
    ...
    data = load_data(args, debug, logger, num_tasks)

    all_result = []
    for fold in range(args.KFold):
        train_data, test_data = cv_random_split(data, fold, 5)
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            result, test_loss = eval(args, model, device, test_loader, criterion)
            print('test: ', result, 'test_loss', test_loss)
            if result[1] > acc:
                acc = result[1]
                best_result=result
        ...
        with open(exp_path+"log.txt", "a+") as f:
            log = 'nofeatures_{}, fold {}, acc, f1, precision, recall: {}'.format(args.dataset,fold,best_result)
            print(log)
            f.write(log)
            f.write('\n')
    ave_result = np.mean(np.array(all_result), 0)
    with open(exp_path + "log.txt", "a+") as f:
        f.write('nofeatures_{}, Average--acc, f1, precision, recall: {}'.format(args.dataset, ave_result))
