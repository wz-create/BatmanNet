...
from BatmanNet.data import MolCollator
from BatmanNet.data import StandardScaler
from BatmanNet.util.metrics import get_metric_func
from BatmanNet.util.nn_utils import initialize_weights, param_count
from BatmanNet.util.scheduler import NoamLR
from BatmanNet.util.utils import build_optimizer, build_lr_scheduler, makedirs, load_checkpoint, get_loss_func, \
    save_checkpoint, build_model
from BatmanNet.util.utils import get_class_sizes, get_data, split_data, get_task_names
from task.predict import predict, evaluate, evaluate_predictions



def train(epoch, model, data, loss_func, optimizer, scheduler,
          shared_dict, output_wise, args: Namespace, n_iter: int = 0,
    ...
    for _, item in enumerate(mol_loader):
        ...
        # Run model
        model.zero_grad()
        preds = model(batch, features_batch, output_wise)
...
def run_training(args: Namespace, time_start, logger: Logger = None) -> List[float]:
    ...
    # Train ensemble of models 训练模型集合
    # ensemble_size=N, 表示同时使用N个模型，最后结果取平均
    for model_idx in range(args.ensemble_size):
        ...
        if args.use_pretrained_model:
            ...
            model = load_checkpoint(args.checkpoint_paths[cur_model], current_args=args, logger=logger, load_model=True)
        else:
            cur_model = model_idx
            debug(f'Building model {model_idx}')
            debug(f'Loading args of model {cur_model} from {args.checkpoint_paths[cur_model]}')
            model = load_checkpoint(args.checkpoint_paths[cur_model], current_args=args, logger=logger, load_model=False)
