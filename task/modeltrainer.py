...
from BatmanNet.model.models import MyGMAE_Task
...
class ModelTrainer:
    ...
    def iter(self, epoch, data_loader, train=True) -> List:
        ...
        cum_iter_count = 0
        node_loss_sum, edge_loss_sum = 0, 0
        # loss_func = self.model.get_loss_func(self.args)

        for _, batch in enumerate(tqdm(data_loader)):
            '''
            batch_graph: f_atoms, f_bonds, a2b, b2a, b2revb, a_scope, b_scope, a2a
            '''
            batch_graph, node_masked_batch, edge_masked_batch, mask_node_target, mask_edge_target = batch
            mask_node_pred, mask_edge_pred = self.model(batch_graph, node_masked_batch, edge_masked_batch)

            loss, node_loss, edge_loss = self.loss_func(mask_node_pred, mask_edge_pred, mask_node_target, mask_edge_target)
        ...
        loss_sum /= cum_iter_count
        node_loss_sum /= cum_iter_count
        edge_loss_sum /= cum_iter_count



        return self.n_iter, loss_sum, node_loss_sum, edge_loss_sum
    ...
    def save(self, epoch, file_path, name=None) -> str:
        ...
        # now = time.localtime()
        # if name is None:
        #     name = "model_%04d_%02d_%02d_%02d_%02d_%02d" % (
