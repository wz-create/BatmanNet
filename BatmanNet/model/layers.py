...
from BatmanNet.util.nn_utils import get_activation_function, select_neighbor_and_aggregate
...
class MPNEncoder(nn.Module):
    ...
    def forward(self,
        ...
        if self.input_layer == 'fc':
            ...
            message = self.act_func(input)  # num_bonds x hidden_size 激活函数
...
class MTBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.parser_name = args.parser_name
        self.atom_messages = atom_messages
        self.hidden_size = hidden_size
    ...
    def forward(self, batch, original_features=None, features_batch=None):
        """
        ...
        :return:
        """
        if self.parser_name == 'pretrain':
            f_atoms, f_bonds, a2b, b2a, b2revb, a_scope, b_scope, a2a, remain_pos_embed = batch
            ...
            if self.atom_messages:
                ...
                if f_atoms.shape[1] != self.hidden_size:
                    ...
                    f_atoms = f_atoms + remain_pos_embed
                    original_features = f_atoms
                ...
                if f_bonds.shape[1] != self.hidden_size:
                    ...
                    f_bonds = f_bonds + remain_pos_embed
                    original_features = f_bonds
        ...
        if self.res_connection:  # 在Multi-Head Attention后面要不要加入残差连接
            if self.atom_messages:
                x_in = f_atoms
            else:
                x_in = f_bonds
        ...
        if self.atom_messages:
            f_atoms = self.sublayer(x_in, x_out)  # LayerNorm + dropout
        else:
            f_bonds = self.sublayer(x_in, x_out)  # LayerNorm + dropout

        if self.parser_name == 'pretrain':
            batch = f_atoms, f_bonds, a2b, b2a, b2revb, a_scope, b_scope, a2a, remain_pos_embed
        else:
            batch = f_atoms, f_bonds, a2b, b2a, b2revb, a_scope, b_scope, a2a
        original_features = original_features
        features_batch = features_batch
        return batch, original_features, features_batch
...
class GTransEncoder(nn.Module):
    def __init__(self,
        ...
        if self.parser_name == 'pretrain':
            self.ffn_atom_from_atom = PositionwiseFeedForward(self.hidden_size * 2,
                                                              self.hidden_size * 4,
                                                              activation=self.activation,
                                                              dropout=self.dropout,
                                                              d_out=self.hidden_size)

            # self.ffn_atom_from_bond = PositionwiseFeedForward(self.hidden_size * 2,
            #                                                   self.hidden_size * 4,
            #                                                   activation=self.activation,
            #                                                   dropout=self.dropout,
            #                                                   d_out=self.hidden_size)
            #
            # self.ffn_bond_from_atom = PositionwiseFeedForward(self.hidden_size * 2,
            #                                                   self.hidden_size * 4,
            #                                                   activation=self.activation,
            #                                                   dropout=self.dropout,
            #                                                   d_out=self.hidden_size)

            self.ffn_bond_from_bond = PositionwiseFeedForward(self.hidden_size * 2,
                                                              self.hidden_size * 4,
                                                              activation=self.activation,
                                                              dropout=self.dropout,
                                                              d_out=self.hidden_size)
    ...
    def atom_bond_transform(self,
        ...
        if to_atom:
            ...
            # return bond_in_bond_out


    def atom_bond_transform_pretrain(self,
                                     to_atom=True,  # False: to bond
                                     atomwise_input=None,
                                     bondwise_input=None,
                                     original_f_atoms=None,
    ...
    def forward(self, batch, node_masked_batch=None, edge_masked_batch=None, remain_atom_pos_embed=None,
                remain_edge_pos_embed=None, features_batch=None):

        # print('parser_name:', self.parser_name)
        if self.parser_name == 'pretrain':
            node_mask_idx_list, node_remain_idx_list, node_mask_fatoms, node_remain_fatoms, node_remain_a2a, \
            node_remain_a2b, node_remain_b2a, node_new_ascope = node_masked_batch

            edge_mask_idx_list, edge_remain_idx_list, edge_mask_fbonds, edge_remain_fbonds, edge_remain_b2a, \
            edge_remain_a2b, edge_remain_b2revb, edge_new_bscope = edge_masked_batch

            f_atoms, f_bonds, a2b, b2a, b2revb, a_scope, b_scope, a2a = batch

            # mask_idx_list和remian_idx_list 目前没有放在cuda上
            if self.cuda or next(self.parameters()).is_cuda:
                f_atoms, f_bonds, a2b, b2a, b2revb, a2a, a_scope, b_scope = f_atoms.cuda(), f_bonds.cuda(), a2b.cuda(), \
                    b2a.cuda(), b2revb.cuda(), a2a.cuda(), a_scope.cuda(), b_scope.cuda()

                node_remain_fatoms, node_remain_a2a, node_remain_a2b, node_remain_b2a, node_new_ascope = \
                    node_remain_fatoms.cuda(), node_remain_a2a.cuda(), node_remain_a2b.cuda(), node_remain_b2a.cuda(), \
                    node_new_ascope.cuda()

                edge_remain_fbonds, edge_remain_b2a, edge_remain_a2b, edge_remain_b2revb, edge_new_bscope = \
                    edge_remain_fbonds.cuda(), edge_remain_b2a.cuda(), edge_remain_a2b.cuda(), edge_remain_b2revb.cuda(), \
                    edge_new_bscope.cuda()
            ...
            node_batch = node_remain_fatoms, f_bonds, node_remain_a2b, node_remain_b2a, b2revb, node_new_ascope, b_scope, \
                         node_remain_a2a, remain_atom_pos_embed
            edge_batch = f_atoms, edge_remain_fbonds, edge_remain_a2b, edge_remain_b2a, edge_remain_b2revb, a_scope, \
                         edge_new_bscope, a2a, remain_edge_pos_embed

            # opt pointwise_feed_forward
            # original_f_atoms = node_remain_fatoms + remain_atom_pos_embed
            # original_f_bonds = edge_remain_fbonds + remain_edge_pos_embed
            ...
            for nb in self.node_blocks:  # atom messages. Multi-headed attention
                node_batch, original_f_atoms, features_batch = nb(node_batch, original_f_atoms, features_batch)
            for eb in self.edge_blocks:  # bond messages. Multi-headed attention
                edge_batch, original_f_bonds, features_batch = eb(edge_batch, original_f_bonds, features_batch)

            #  Multi-Head-Attention输出的结果
            #  node_batch, edge_batch, 包含的变量：f_atoms, f_bonds, a2b, b2a, b2revb, a_scope, b_scope, a2a, remain_pos_embed
            atom_output, _, _, _, _, _, _, _, _ = node_batch  # atom hidden states
            _, bond_output, _, _, _, _, _, _, _ = edge_batch  # bond hidden states

            atom_embeddings = self.atom_bond_transform_pretrain(to_atom=True,  # False: to bond
            ...
            bond_embeddings = self.atom_bond_transform_pretrain(to_atom=False,  # False: to bond
                                                                atomwise_input=atom_output,
                                                                bondwise_input=bond_output,
                                                                original_f_atoms=original_f_atoms,
                                                                original_f_bonds=original_f_bonds,
            ...
            # atom_embeddings = atom_output
            # bond_embeddings = bond_output
...
class GTransDecoder(nn.Module):
    def __init__(self,
                 ...
                 cuda=True,
                 res_connection=False):
        super(GTransDecoder, self).__init__()
        # For the compatibility issue.
        if atom_emb_output is False:
            atom_emb_output = None
        if atom_emb_output is True:
        ...
        # self.atom_from_atom_sublayer = SublayerConnection(size=node_fdim, dropout=self.dropout)
        # # self.atom_from_bond_sublayer = SublayerConnection(size=edge_fdim, dropout=self.dropout)
        # # self.bond_from_atom_sublayer = SublayerConnection(size=node_fdim, dropout=self.dropout)
        # self.bond_from_bond_sublayer = SublayerConnection(size=edge_fdim, dropout=self.dropout)
        ...
        self.atom_fc = nn.Linear(self.hidden_size, node_fdim)
        self.bond_fc = nn.Linear(self.hidden_size, edge_fdim)
    ...
    def forward(self, node_embedding, edge_embedding, graph_batch, features_batch=None):
        f_atoms, f_bonds, a2b, b2a, b2revb, a_scope, b_scope, a2a = graph_batch
        pos_embed = None  # 在解码时，位置编码为None

        if self.cuda or next(self.parameters()).is_cuda:
            node_embedding, edge_embedding, f_atoms, f_bonds, a2b, b2a, b2revb, a_scope, b_scope, a2a = \
                node_embedding.cuda(), edge_embedding.cuda(), f_atoms.cuda(), f_bonds.cuda(), a2b.cuda(), b2a.cuda(), \
                b2revb.cuda(), a_scope.cuda(), b_scope.cuda(), a2a.cuda()

        node_batch = node_embedding, f_bonds, a2b, b2a, b2revb, a_scope, b_scope, a2a, pos_embed
        edge_batch = f_atoms, edge_embedding, a2b, b2a, b2revb, a_scope, b_scope, a2a, pos_embed
        # node_edge_batch = node_embedding, edge_embedding, a2b, b2a, b2revb, a_scope, b_scope, a2a, pos_embed

        original_f_atoms = node_embedding
        original_f_bonds = edge_embedding
        # Note: features_batch is not used here.
        for nb in self.node_blocks:  # atom messages. Multi-headed attention
            node_batch, original_f_atoms, features_batch = nb(node_batch, original_f_atoms, features_batch)
        for eb in self.edge_blocks:  # bond messages. Multi-headed attention
        ...
        bond_embeddings = self.atom_bond_transform(to_atom=False,  # False: to bond
                                                   ...
                                                   b2a=b2a,
                                                   b2revb=b2revb)
        atom_embeddings = self.atom_fc(atom_embeddings)
        bond_embeddings = self.bond_fc(bond_embeddings)
