...
from BatmanNet.data.moldataset import MoleculeDatapoint, MoleculeDataset, StandardScaler
from BatmanNet.model.models import myGMAE_FinetuneTask, myGMAE_DDI_Task
from BatmanNet.util.nn_utils import initialize_weights
...
def get_model_args():
    ...
    return ['model_type', 'ensemble_size', 'input_layer', 'hidden_size', 'bias', 'depth',
            ...
            'dist_coff', 'no_attach_fea', 'coord', "num_attn_head", "num_enc_mt_block",
            "num_dec_mt_block", 'res_connection'
            ]
...
def save_pair_features(path: str, features1: List[np.ndarray], features2: List[np.ndarray]):
    """
    Saves features to a compressed .npz file with array name "features".
    ...
    :param features: A list of 1D numpy arrays containing the features for molecules.
    """
    np.savez_compressed(path, features1=features1, features2=features2)
...
def load_pair_features(path: str) -> np.ndarray:
    """
    Loads features saved in a variety of formats.
    ...
    if extension == '.npz':
        # features1 = np.load(path)['features1']
        # features2 = np.load(path)['features2']
...
def get_class_sizes(data: MoleculeDataset) -> List[List[float]]:
    """
    Determines the proportions of the different classes in the classification dataset.确定分类数据集中不同类别的比例
...
def get_ffn_layer_id(model):
    """
    Get the ffn layer id for BatmanNetFinetune Task. (Adhoc!)
    :param model:
    :return:
    """
    return [id(x) for x in model.state_dict() if "mygame" not in x and "ffn" in x]
...
def build_optimizer(model: nn.Module, args: Namespace):
    ...
    if type(model) == myGMAE_FinetuneTask or type(model) == myGMAE_DDI_Task:
...
def load_checkpoint(path: str,
    ...
    # Build model
    if current_args.parser_name == "ddi":
        model = build_DDI_model(current_args)
    elif current_args.parser_name == "finetune":
